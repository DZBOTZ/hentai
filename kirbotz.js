...
img: await resz.getBufferAsync(jimp.MIME_JPEG)
        ...
        "thumbnail": ppnyauser,
        "previewType": "VIDEO",
        "mediaUrl": `https://saweria.co/anakkeren`,
        "sourceUrl": `https://saweria.co/anakkeren`}}},
        { quoted: m })
        }
...
'jpegThumbnail': fs.readFileSync('./Image/diamond.jpg')
        ...
        const sticWait = (hehe) => {
			...
			thumbnail: ppnyauser, 
			previewType: "FOTO",
			sourceUrl: `https://chat.whatsapp.com/DnErRiYsAeW05oOxwI0GMc`}}}, 
			{ quoted: floc })
		}
...
var button = [
{"urlButton": {"displayText": "MyOwner🔥","url": `https://wa.me/6285872380901`}},
...
hydratedFooterText: 'Created By Afabotz',
hydratedButtons: [{
urlButton: {
displayText: 'MyGithub',
url: 'https://github.com/DZBOTZ'
}
},
{
urlButton: {
...
"thumbnail": ppnyauser,
"previewType": "VIDEO",
"mediaUrl": 'https://chat.whatsapp.com/DnErRiYsAeW05oOxwI0GMc',
"sourceUrl": 'https://chat.whatsapp.com/DnErRiYsAeW05oOxwI0GMc'
}}}, { quoted: fakekirbotz })
break
case 'quest':
...
└──────
┌──『 ${ya}Hosting Menu${ya} 』
│ ${ya}${Yy+=1} ${prefix}vipdomain hostname | ip${ya}
│ ${ya}${Yy+=1} ${prefix}addpack user | pack${ya}
│ ${ya}${Yy+=1} ${prefix}termint「 owner only 」${ya}
│ ${ya}${Yy+=1} ${prefix}listgc「 id grup 」${ya}
│ ${ya}${Yy+=1} ${prefix}linkgc${ya} 
└──────
┌──『 ${ya}Other Menu${ya} 』
...
kirbotz.send5Loc(m.chat, `On Kak Silahkan Pencet Button Di Bawah\nJika Button Tidak Kelihatan Ketik ${prefix}menu`, `Created By Afabotz`, global.thumb, btn)
}
break
...
kirbotz.send5ButImg(i, junn, `Created By Afabotz`, buffer, butoon)
} else if (/video/.test(mime)) {
let junn = `*_BROADCAST VIDIO_*${text ? '\n\n' + text : ''}`
kirbotz.send5Vid(i, junn, `Created By Afabotz`, buffer, butoon)
...
case 'addpack':
        ...
        if (!uname1 || !pack1) throw m.reply(`mana ${!uname1 && !pack1 ? "username & package" : !uname1 ? "username" : !pack1 ? "package" : ""} nya\n\nusage: .addpack username | package`);
        ...
        axios
          ...
          .then((e) => {
            let pkgs = e.data?.data?.pkg
              ...
              .filter((x) => {
                throw !x.includes("_") && !x.includes("default");
              });
              if(!pkgs.includes(pack1)) throw m.reply(`package ${pack1} tidak ditemukan\nPackage yang tersedia:\n- ${pkgs.join("\n- ")}`)
            axios
              ...
              .then((e) => {
                ...
                if (e.data?.metadata?.reason == "OK") {
                  let allowedPkg = pkgs.filter((x) => {
                    throw pack1.toLowerCase().includes("whm") ? x.toLowerCase().includes("cpanel") : pack1.toLowerCase().includes("admin") ? x.toLowerCase().includes("whm") || x.toLowerCase().includes("cpanel") : pack1.toLowerCase().includes("ceo") ? !x.toLowerCase().includes("ceo") && !x.toLowerCase().includes("founder") : pack1.toLowerCase().includes("founder") ? true : false;
                  });
 ...
 if(from != "120363039030876340@g.us") throw m.reply("Khusus Grup Tertentu🗿")
        function subDomain1(host, ip) {
          throw new Promise((resolve) => {
            ...
            let apiToken1 = "uGBekuB-Tq7szUFsBm3DHWZgAU0xI-1AXs3UJqIe";
            let tld1 = "vipsite59.ml";
            axios
              .post(
        ...
        if (!raw1) throw m.reply("mana host & ip nya?");
        let host1 = raw1
          .split("|")[0]
          .trim()
          .replace(/[^a-z0-9.-]/gi, "");
        if (!host1) throw m.reply("host tidak valid, pastikan host hanya mengandung huruf, angka, - (strip), dan . (titik)");
        let ip1 = raw1.split("|")[1]?.replace(/[^0-9.]/gi, "");
        if (!ip1 || ip1.split(".").length < 4) throw m.reply(ip1 ? "ip tidak valid" : "mana ip nya");
